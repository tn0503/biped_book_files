...
int16_t leftAction[][13] = {
  { 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 8},
  { 90, 90, 90,100, 90, 90, 90, 90,100, 90, 90, 90, 8}, //右重心
  {100, 90, 90,100, 90, 90, 90, 90,100, 90, 90, 90, 8},
  { 90, 90, 90, 80, 90, 90, 90, 90, 80, 90, 90, 80, 8}, //左重心
...
void loop() {
  ...
  // フレーム処理
  divCounter++;
  if(divCounter >= leftAction[nextKeyFrame][12]) {
    divCounter = 0;
  ...
  // サーボ角度計算
  for(int i = 0; i < 12; i++) {
    tempAngles[i] = leftAction[keyFrame][i] +
              int8_t((leftAction[nextKeyFrame][i] - leftAction[keyFrame][i])
               * divCounter / leftAction[nextKeyFrame][12]);
  }
  // サーボ信号出力
  for(i = 0; i < 12; i++) {
