...
int16_t rtrnAction[][13] = {
{90,100,100, 90,110, 90, 70,110, 90,100,100, 90, 8},
  {90, 90, 90, 90, 90, 90, 90, 80, 90,100,100, 90, 8}, //中重心
  {90, 90, 90, 90, 70, 90, 90, 70, 90, 90, 90, 90, 4},
  {90, 90, 90, 90, 70, 90, 90, 70, 90, 80, 80, 90, 4}, //右足前
  {90, 80, 80, 90, 70,110, 90, 70, 90, 80, 80, 90, 8},
  {90, 80, 80, 90, 90, 90, 90, 90,100, 90, 90, 90, 8}, //右重心
  {90, 90, 90, 90,110, 90, 90,110,100, 90, 90, 90, 4},
  {90,100,100, 90,110, 90, 90,110,100, 90, 90, 90, 4}  //左足前
};
//uint8_t divide = 10;
uint8_t divCounter;
uint8_t keyFrame;
...
void loop() {
  ...
  // フレーム処理
  divCounter++;
  if(divCounter >= rtrnAction[nextKeyFrame][12]) {
    divCounter = 0;
  ...
  // サーボ角度計算
  for(int i = 0; i < 12; i++) {
    tempAngles[i] = rtrnAction[keyFrame][i] +
              int8_t((rtrnAction[nextKeyFrame][i] - rtrnAction[keyFrame][i])
               * divCounter / rtrnAction[nextKeyFrame][12]);
  }
  // サーボ信号出力
  for(i = 0; i < 12; i++) {
